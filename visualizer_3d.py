...
class Visualizer(imgui_window.ImguiWindow):
    def __init__(self, capture_dir=None):
        ...
        self.cam_widget          = camera_widget.CameraWidget(self)
...
class AsyncRenderer:
    ...
    def _set_args_sync(self, **args):
        if self._renderer_obj is None:
            self._renderer_obj = renderer_3d.Renderer()
    ...
    def _process_fn(args_queue, result_queue):
        renderer_obj = renderer_3d.Renderer()
        cur_args = None
        cur_stamp = None
        while True:
            ...
            if args != cur_args or stamp != cur_stamp:
                ...
                if 'error' in result:
                    result.error = renderer_3d.CapturedException(result.error)
